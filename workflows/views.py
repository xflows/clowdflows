...
def index(request):
    ...
    return render(request, 'index.html',
                  {'categories': categories, 'user': request.user, 'user_categories': user_categories,
        ...
        if request.POST.has_key("abstractwidget_id"):
            ...
            if aw.type == 'regular':
                ...
                if (workflow.user == request.user):
                    ...
                    workflows.models.copy_workflow(aw.workflow_link, request.user, widget_conversion, input_conversion,
                                                   output_conversion, w)
                    w.defered_outputs = w.outputs.defer("value").all()
                    w.defered_inputs = w.inputs.defer("value").all()
                    return render(request, 'widgets.html', {'widgets': [w, ]})
    ...
    if request.is_ajax() or DEBUG:
        ...
        if (i.widget.workflow == o.widget.workflow and i.widget.workflow.user == request.user):
            ...
            if not checkForCycles(i.widget, i.widget):
                ...
                data = simplejson.dumps({'message': message, 'success': success, 'deleted': deleted, 'added': added,
                                         'input_id': request.POST['input_id'], 'output_id': request.POST['output_id']})
            ...
            data = simplejson.dumps({'message': message, 'success': success, 'deleted': deleted, 'added': added,
                                     'input_id': request.POST['input_id'], 'output_id': request.POST['output_id'],
                                     'refresh': refresh, 'refreshworkflow': refreshworkflow})
            return HttpResponse(data, mimetype)
        else:
    ...
    if request.is_ajax() or DEBUG:
        ...
        if (w.workflow.user == request.user):
            ...
            return render(request, 'configuration.html',
                          {'widget': w, 'inputs': inputs, 'parameters': parameters, 'outputs': outputs,
    ...
    if request.is_ajax() or DEBUG:
        ...
        if (widget.workflow.user == request.user):
            ...
            data = simplejson.dumps(
                {'changed': changed, 'reordered': reordered, 'deletedConnections': deletedConnections})
            mimetype = 'application/javascript'
            return HttpResponse(data, mimetype)
        else:
            return HttpResponse(status=400)
    ...
    if request.is_ajax() or DEBUG:
        ...
        if (w.workflow.user == request.user):
            ...
            try:
                ...
                workflow_link = False
            data = simplejson.dumps(
                {'workflow_link': workflow_link, 'workflow_link_id': workflow_link_id, 'rename_inputs': rename_inputs,
            ...
            try:
                ...
                for inp in w.inputs.filter(required=True, parameter=False):
                    if inp.connections.count() == 0:
                        if inp.multi_id == 0:
                            raise Exception(
                                "The input " + str(inp) + " must have something connected to it in order to run.")
                        else:
                ...
                for mid in multi_satisfied.keys():
                    if multi_satisfied[mid][1] == False:
                        raise Exception("The input " + multi_satisfied[mid][
                            0] + " must have something connected to it in order to run.")
                ...
                if not w.abstract_widget is None:
                    if w.abstract_widget.interactive:
                        ...
                        w.save()
                        data = simplejson.dumps(
                            {'status': 'interactive', 'message': 'Widget ' + w.name + ' needs your attention.',
                             'widget_id': w.id})
                    elif w.abstract_widget.visualization_view != '':
                        data = simplejson.dumps(
                            {'status': 'visualize', 'message': 'Visualizing widget ' + w.name + '.', 'widget_id': w.id})
                    else:
                        data = simplejson.dumps(
                            {'status': 'ok', 'message': 'Widget ' + w.name + ' executed successfully.'})
                else:
    ...
    if request.is_ajax() or DEBUG:
        ...
        if (w.workflow.user == request.user):
            try:
                ...
                data = simplejson.dumps({'status': 'error',
                                         'message': 'Error occurred when trying to reset the widget ' + w.name + ': ' + str(
                                             type(e)) + ' ' + str(e)})
            return HttpResponse(data, mimetype)
        else:
            return HttpResponse(status=400)
...
def widget_results(request):
    ...
    if request.is_ajax() or DEBUG:
        ...
        if (w.workflow.user == request.user):
            ...
            return render(request, 'visualizations/result_viewer.html',
                          {'widget': w, 'input_dict': input_dict, 'output_dict': output_dict})
        else:
            return HttpResponse(status=400)
    else:
        return HttpResponse(status=400)
        ...
        if (w.workflow.user == request.user):
            try:
                ...
                if w.abstract_widget.visualization_view != '':
                    ...
                    data = simplejson.dumps(
                        {'status': 'ok', 'message': 'Widget ' + w.name + ' executed successfully.', 'widget_id': w.id})
            except Exception, e:
                mimetype = 'application/javascript'
                w.error = True
                w.running = False
...
def export_package(request, packages):
    ...
    updateuid = (
    request.GET.get('updateuid', 'False').lower() == 'true' or request.GET.get('u', 'False').lower() == 'true')
