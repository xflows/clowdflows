...
def dumpObj(obj, maxlen=80, lindent=24, maxspew=600):
    ...
    out=out+"\n"+prettyPrintCols( ('Documentation string:',
    ...
    if builtins:
        ...
        out=out+"\n"+prettyPrintCols( ('Built-in Methods:',
    ...
    for (attr, val) in attrs:
        value = truncstring(printval(val), maxspew)
        strAttr = prettyPrintCols(('', attr, value), tabbedwidths, ' ')
    ...
    if len(att)>0:
        out += "\n"+'Attributes:'+att
    if len(att)>0 and len(bmth)>0:
        out += "\n"
    if len(bmth)>0:
        out += "\n"+'Attributes (Bound Methods):'+bmth

    return out

def printval(val):
    if is_netsequence(val):
...
def is_netsequence(arg):
    return is_sequence(arg) and hasattr(arg, 'ForEach') and hasattr(arg, 'GetEnumerator')
