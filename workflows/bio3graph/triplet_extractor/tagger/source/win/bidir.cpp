...
bidir_postag(const string & s, const vector<ME_Model> & vme, const vector<ME_Model> & chunking_vme, bool dont_tokenize)
{
  ...
  PyObject *tagList = PyList_New((Py_ssize_t) vt.size());

  //string tmp;
  for (size_t i = 0; i < vt.size(); i++) {
    PyObject *result = PyTuple_New((Py_ssize_t) 5);
    PyTuple_SET_ITEM(result, (Py_ssize_t) 0, PyString_FromString(vt[i].str.c_str()));
    PyTuple_SET_ITEM(result, (Py_ssize_t) 1, PyString_FromString(base_form(vt[i].str, vt[i].prd).c_str()));
    PyTuple_SET_ITEM(result, (Py_ssize_t) 2, PyString_FromString(vt[i].prd.c_str()));
    PyTuple_SET_ITEM(result, (Py_ssize_t) 3, PyString_FromString(vt[i].cprd.c_str()));
    PyTuple_SET_ITEM(result, (Py_ssize_t) 4, PyString_FromString(vt[i].ne.c_str()));

    PyList_SET_ITEM(tagList, (Py_ssize_t) i, result);
  }
  return tagList;
}
